#pragma once

// WORLD & WINDOW CONFIGURATION
#define PIXELS_PER_METER 50.f
#define WORLD_HEIGHT 2250.f
#define WORLD_WIDTH 4000.f
#define DEF_HEIGHT 450.f
#define DEF_WIDTH 800.f
#define WALL_THICKNESS 30.f
#define HALF_WALL_THICKNESS (WALL_THICKNESS / 2)
#define WALL_COLOR sf::Color(140, 140, 140, 255)
#define CONNECTOR_COLOR sf::Color(200, 200, 200, 255)
#define CONNECTOR_THICKNESS 5.f
#define ZOOM_STEP 0.025f
#define PAN_SPEED 15.f

// GRID CONFIGURATION
#define GRID_MAJOR_SPACING 5.0f
#define GRID_MINOR_SPACING 1.0f
#define GRID_MINOR_COLOR sf::Color(50, 50, 50, 255)
#define GRID_MAJOR_COLOR sf::Color(80, 80, 80, 255)
#define GRID_AXIS_COLOR sf::Color(120, 120, 120, 255)
#define BACKGROUND_COLOR sf::Color(30, 30, 30, 255)

// PATH TRACE CONFIGURATION
#define PATH_TRACE_POINT_RADIUS 2.5f
#define PATH_TRACE_POINT_COLOR sf::Color(100, 150, 255, 255)
#define PATH_TRACE_FADE_COLOR sf::Color(100, 150, 255, 50)
#define MIN_PATH_TRACE_FADE_TIME 0.1f
#define MAX_PATH_TRACE_FADE_TIME 10.0f
#define PATH_TRACE_FADE_TIME_STEP 0.1f
#define DEFAULT_PATH_TRACE_FADE_TIME 2.0f

// FORCE ARROW CONFIGURATION
#define MIN_SIZE 0.1f
#define FORCE_ARROW_SCALE 1.0f
#define FORCE_ARROW_THICKNESS 2.0f
#define APPLIED_FORCE_ARROW_COLOR sf::Color(255, 0, 0, 255)
#define GRAVITY_FORCE_ARROW_COLOR sf::Color(0, 255, 0, 255)
#define COLLISION_FORCE_ARROW_COLOR sf::Color(0, 0, 255, 255)
#define FRICTION_FORCE_ARROW_COLOR sf::Color(0, 255, 255, 255)
#define DRAG_FORCE_ARROW_COLOR sf::Color(255, 255, 0, 255)

// UI CONFIGURATION
#define Y_ITEM_SPACING 6.0f
#define ATTACHMENT_POINT_RADIUS 2.5f

// TOOLS CONFIGURATION
#define TOOLS_ICON_SIZE 32
#define TOOL_BG_COLOR sf::Color(41, 74, 122, 102)
#define TOOL_SELECT_COLOR sf::Color(66, 150, 250, 200)

// ENGINE CONFIGURATION
#define DEFAULT_SOLVER RK4

#define DEFAULT_CALC_FREQ 240 // Hz
#define MAX_CALC_FREQ 1000    // Hz
#define MIN_CALC_FREQ 30      // Hz
#define CALC_FREQ_STEP 10     // Hz

#define MAX_DT 0.05f             // seconds
#define TEST_SCENE_DURATION 3.0f // seconds

#define MAX_FPS 120

// PHYSICS CONFIGURATION
#define MIN_GRAVITY -50.0f
#define MAX_GRAVITY 50.0f
#define GRAVITY_STEP 0.5f
#define DEFAULT_GRAVITY Vec2(0.0f, 10.0f)

#define MIN_AIR_DENSITY 0.0f
#define MAX_AIR_DENSITY 10.0f
#define AIR_DENSITY_STEP 0.1f
#define DEFAULT_AIR_DENSITY 1.225f

// OBJECT & TOOL PROPERTIES LIMITS & DEFAULTS
#define MIN_PERCENTAGE 0.0f
#define MAX_PERCENTAGE 100.0f
#define PERCENTAGE_STEP 1.0f
#define DEFAULT_PERCENTAGE 50.0f

#define MIN_LENGTH 0.1f
#define MAX_LENGTH 10.0f
#define LENGTH_STEP 0.05f
#define DEFAULT_LENGTH 0.25f

#define MIN_ANGLE 0.0f
#define MAX_ANGLE 360.0f
#define ANGLE_STEP 5.0f
#define DEFAULT_ANGLE 0.0f

#define MIN_DENSITY 0.1f
#define MAX_DENSITY 100.0f
#define DENSITY_STEP 0.5f
#define DEFAULT_DENSITY 1.0f

#define MIN_FORCE 0.0f
#define MAX_FORCE 500.0f
#define FORCE_STEP 5.0f
#define DEFAULT_FORCE 50.0f
#define MAX_ATTENUATION 1.0f
#define FORCE_SCALE 0.05f

#define MIN_DRAG 0.0f
#define MAX_DRAG 1.0f
#define DRAG_STEP 0.02f
#define DEFAULT_DRAG 0.0f

#define MIN_FRICTION 0.0f
#define MAX_FRICTION 1.0f
#define FRICTION_STEP 0.02f
#define DEFAULT_FRICTION 0.0f

#define MIN_RESTITUTION 0.0f
#define MAX_RESTITUTION 1.0f
#define RESTITUTION_STEP 0.02f
#define DEFAULT_RESTITUTION 0.7f

#define MIN_STIFFNESS 0.0f
#define MAX_STIFFNESS 1000.0f
#define STIFFNESS_STEP 5.0f
#define DEFAULT_STIFFNESS 25.0f

#define MIN_DAMPING 0.0f
#define MAX_DAMPING 100.0f
#define DAMPING_STEP 1.0f
#define DEFAULT_DAMPING 0.0f